...
function restore_doPostgresRestore() {
  ...
  if get_user_approval; then
    ...
    # Our initdb script that runs when the container launches will have already created the
    # databases and users.
    ...
    if [ "$CONTAINER_RUNTIME" == "podman" ]; then
      ...
      debug "`compose_client exec -T --user $plextrac_user_id $postgresComposeService --env PGPASSWORD=$POSTGRES_PASSWORD \

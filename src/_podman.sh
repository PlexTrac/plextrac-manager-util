...
function plextrac_install_podman() {
  ...
  serviceValues[plextracnginx-env_vars]="-e UPSTREAM_CLOUD_BUCKET=${UPSTREAM_CLOUD_BUCKET:-cloud} -e UPSTREAM_CLOUD_PREFIX=${UPSTREAM_CLOUD_PREFIX:-uploads} -e MINIO_ENABLED=${MINIO_ENABLED:-true} -e UPSTREAM_CLOUD_HOST=${UPSTREAM_CLOUD_HOST:-minio} -e USE_CUSTOM_CERT=${${USE_CUSTOM_CERT-} -e WAF_ENABLED=${WAF_ENABLED:-false }"
